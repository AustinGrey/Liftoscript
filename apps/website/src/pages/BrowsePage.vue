<template>
	<div class="browse">
		<section class="hero">
			...
			<p class="hero-lede">
				{{ programs.length }} real-world workout programs, written in Liftoscript. Open any one to
				edit its plan, dial in a lifter's settings, and simulate how the training progresses session
				by session.
			</p>
		</section>
			...
			<div class="filter-row">
				...
				<label class="filter">
					...
					<select v-model="filters.frequency">
						<option value="">Any</option>
						<option v-for="f in frequencies" :key="f" :value="f">{{ f }}&times;/week</option>
					</select>
				</label>
				<label class="filter">
		...
		<div v-if="filtered.length" class="program-grid">
			<ProgramCard v-for="program in filtered" :key="program.slug" :program="program" />
		</div>
		<div v-else class="empty-state">
			<p>No programs match those filters.</p>
			<button type="button" class="reset" @click="resetFilters">Clear filters</button>
		</div>
	</div>
</template>
