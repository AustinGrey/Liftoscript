	...
	<div class="settings-panel">
		<section class="settings-block">
			...
			<div class="segmented" role="group" aria-label="Units">
				<button type="button" :class="{ active: state.units === 'lb' }" @click="setUnits('lb')">
					lb
				</button>
				<button type="button" :class="{ active: state.units === 'kg' }" @click="setUnits('kg')">
					kg
				</button>
			</div>
		</section>
		...
		<section class="settings-block">
			...
			<div class="sessions-control">
				<input type="range" min="1" max="60" v-model.number="state.numberOfSessions" />
		...
		<section class="settings-block">
			<h3>Starting 1RM</h3>
			<p class="hint">
				The training max each lift is based on. Leave blank to use the library's default starting
				weight.
			</p>
			<div v-if="exercises.length === 0" class="empty-note">No exercises detected yet.</div>
