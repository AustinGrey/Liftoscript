...
function switchWeightsInPlanToUnit(programNode: SourcedSyntaxNode, settings: ISettings): string {
...
function switchWeightsToUnit(programNode: SourcedSyntaxNode, toUnit: IUnit): string {
	const cursor = programNode.cursor();
	let script = programNode.source;
	let shift = 0;
	do {
...
export function PlannerTestUtils_finish(
	...
	return Program_runAllFinishDayScripts(program, nextHistoryRecord, stats, settings);
}
