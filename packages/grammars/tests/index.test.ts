...
import { dw, type IDynamicWeight, type IWeight, w } from "@/quantities/weight.ts";
import {
	IProgramMode,
	type IProgramState,
		...
		describe.each(cases)("Result is $result for case %#: $description", (case_) => {
			...
			test("new system", () => {
				...
				if ("result" in case_) {
					...
					expect
						.soft(round(output.data, rounder), "Script should evaluate to the expected result")
						.toEqual(case_.result);
				}
				if (finalState) {
					expect.soft(state, "State after evaluation completes should match").toEqual(finalState);
				}
			});
		});
	},
