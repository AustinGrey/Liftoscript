...
export type IExercisePointsColl = Partial<
  Record<string, IScreenMusclePointsColl>
>;
...
const muscleToScreenMuscleMapping = ObjectUtils_keys(
  screenMuscleToMuscleMapping,
).reduce<Record<IMuscle, IScreenMuscle[]>>((memo, screenMuscle) => {
...
export function Muscle_normalizePoints(points: IPoints): IPoints {
  const maxValues = {
    ...
    hypertrophy: ObjectUtils_findMaxValue(
      points.screenMusclePoints.hypertrophy,
    ),
  };
  return {
    screenMusclePoints: {
      strength: normalize(
        points.screenMusclePoints.strength,
        maxValues.strength,
      ),
      hypertrophy: normalize(
        points.screenMusclePoints.hypertrophy,
        maxValues.hypertrophy,
      ),
    },
    exercisePoints: {
      strength: ObjectUtils_keys(
        points.exercisePoints.strength,
      ).reduce<IExercisePointsColl>((memo, key) => {
        memo[key] = normalize(
          points.exercisePoints.strength[key]!,
          maxValues.strength,
        );
        return memo;
      }, {}),
      hypertrophy: ObjectUtils_keys(
        points.exercisePoints.hypertrophy,
      ).reduce<IExercisePointsColl>((memo, key) => {
        memo[key] = normalize(
          points.exercisePoints.hypertrophy[key]!,
          maxValues.hypertrophy,
        );
        return memo;
      }, {}),
...
export function Muscle_normalizeUnifiedPoints(
  points: IUnifiedPoints,
): IUnifiedPoints {
  ...
  return {
    ...
    exercisePoints: ObjectUtils_keys(
...
export function Muscle_combinePoints(points: IPoints): IPoints {
  return {
    screenMusclePoints: {
      strength: ObjectUtils_keys(
        points.screenMusclePoints.strength,
      ).reduce<IScreenMusclePointsColl>((memo, key) => {
...
export function Muscle_getScreenMusclesFromMuscle(
  muscle: IMuscle,
  settings: ISettings,
): (IScreenMuscle | string)[] {
  ...
  const matchingMuscleGroups = availableMuscleGroups.filter((mg) => {
    const muscles =
      settings.muscleGroups.data[mg]?.muscles ??
      screenMuscleToMuscleMapping[mg as IScreenMuscle] ??
      [];
...
export function Muscle_getMuscleGroupName(
  muscleGroup: IScreenMuscle,
  settings: ISettings,
): string {
  return (
    settings.muscleGroups.data[muscleGroup]?.name ??
    StringUtils_capitalize(muscleGroup)
  );
}

export function Muscle_isDefaultMuscles(
  muscleGroup: IScreenMuscle,
  muscles: IMuscle[],
): boolean {
...
export function Muscle_getAvailableMuscleGroups(
  settings: ISettings,
): IScreenMuscle[] {
  return [
    ...screenMuscles.filter((sm) => !settings.muscleGroups.data[sm]?.isHidden),
    ...Object.keys(settings.muscleGroups.data).filter(
      (k) => !(screenMuscles as readonly string[]).includes(k),
    ),
  ];
}
...
export function Muscle_getMusclesFromScreenMuscle(
  muscleGroup: IScreenMuscle | string,
  settings: ISettings,
): IMuscle[] {
  const availableMuscleGroups = Muscle_getAvailableMuscleGroups(settings);
...
export function Muscle_isBuiltInMuscleGroup(
  muscleGroup: IScreenMuscle | string,
): boolean {
  return (screenMuscles as readonly string[]).includes(muscleGroup);
}

export function Muscle_getPointsForProgram(
  ...
  return program.weeks[0].days.reduce(
    (memo, day) =>
      mergePoints(memo, Muscle_getPointsForDay(program, day, stats, settings)),
    {
      screenMusclePoints,
      exercisePoints,
    },
  ...
  return dayExercises.reduce(
    (memo, exercise) => {
      return mergeUnifiedPoints(
        memo,
        Muscle_getUnifiedPointsForExercise(
          program,
          exercise,
          programDay.dayData,
          settings,
...
export function Muscle_getUnifiedPointsForExercise(
  ...
  const historyEntry = Program_nextHistoryEntry(
    program,
    dayData,
    0,
    programExercise,
  ...
  const targetMuscles = Exercise_targetMuscles(
    programExercise.exerciseType,
    settings,
  );
  const synergistMuscles = Exercise_synergistMuscles(
    programExercise.exerciseType,
    settings,
  );
  const screenTargetMuscles = Array.from(
    new Set(targetMuscles.flatMap((t) => muscleToScreenMuscleMapping[t] || [])),
  );
  const screenSynergistMuscles = Array.from(
    new Set(
      synergistMuscles.flatMap((t) => muscleToScreenMuscleMapping[t] || []),
    ),
  );
  const synergistMuscleGroupToMultiplier =
  ...
  for (const _set of historyEntry.sets) {
    ...
    for (const muscle of screenSynergistMuscles) {
      ...
      screenMusclePoints[muscle]! +=
        (synergistMuscleGroupToMultiplier[muscle] ??
          settings.planner.synergistMultiplier) * 100;
      exercisePoints[id] = exercisePoints[id] || {};
      exercisePoints[id]![muscle] = exercisePoints[id]![muscle] || 0;
    ...
    if ((set.reps ?? 0) >= 8) {
      for (const muscle of screenTargetMuscles) {
        screenMusclePoints.hypertrophy[muscle] =
          screenMusclePoints.hypertrophy[muscle] || 0;
        ...
        exercisePoints.hypertrophy[id]![muscle] =
          exercisePoints.hypertrophy[id]![muscle] || 0;
      ...
      for (const muscle of screenTargetMuscles) {
        screenMusclePoints.strength[muscle] =
          screenMusclePoints.strength[muscle] || 0;
        ...
        exercisePoints.strength[id]![muscle] =
          exercisePoints.strength[id]![muscle] || 0;
...
export function Muscle_mergeScreenMusclePoints(
  ...
  return ObjectUtils_combinedKeys(a, b).reduce<IScreenMusclePointsColl>(
    (memo, screenMuscle) => {
  ...
  if (isBuiltin) {
    return {
      ...
      data: {
        ...
        [muscleGroup]: {
          ...muscleGroupSettings.data[muscleGroup],
          isHidden: true,
        },
      },
    };
...
function mergeExercisePoints(
  a: IExercisePointsColl,
  b: IExercisePointsColl,
): IExercisePointsColl {
  return ObjectUtils_combinedKeys(a, b).reduce<IExercisePointsColl>(
    (memo, exerciseId) => {
...
function mergePoints(oldPoints: IPoints, newPoints: IPoints): IPoints {
  return {
    ...
    exercisePoints: {
      strength: mergeExercisePoints(
        oldPoints.exercisePoints.strength,
        newPoints.exercisePoints.strength,
      ),
      hypertrophy: mergeExercisePoints(
        oldPoints.exercisePoints.hypertrophy,
        newPoints.exercisePoints.hypertrophy,
      ),
    },
  };
}

function mergeUnifiedPoints(
  oldPoints: IUnifiedPoints,
  newPoints: IUnifiedPoints,
): IUnifiedPoints {
  return {
    screenMusclePoints: Muscle_mergeScreenMusclePoints(
      oldPoints.screenMusclePoints,
      newPoints.screenMusclePoints,
    ),
    exercisePoints: mergeExercisePoints(
      oldPoints.exercisePoints,
      newPoints.exercisePoints,
    ),
  };
}
//#endregion
...
//#region Stubs
function ExerciseImageUtils_exists(...args: any[]): boolean {
  return false;
}
//#endregion
