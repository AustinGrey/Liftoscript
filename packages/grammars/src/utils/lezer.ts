 ...
 */
import { IterMode, NodeProp, NodeType, type SyntaxNode, Tree, TreeCursor } from "@lezer/common";
...
export interface SourcedSyntaxNode {
	...
	enter(pos: number, side: -1 | 0 | 1, mode?: IterMode): SourcedSyntaxNode | null;
...
function bindNode(node: SyntaxNode, getSource: () => string): SourcedSyntaxNode {
	...
	return {
		...
		enterUnfinishedNodesBefore: (...args) => recurseSure(node.enterUnfinishedNodesBefore(...args)),
 ...
 */
export function parseBound(parser: LRParser, script: string): SourcedSyntaxNode {
...
export function isSourcedSyntaxError(error: unknown): error is SourcedSyntaxError {
...
export function nodeError(
	...
	return new SourcedSyntaxError(`${message} (${line}:${offset})`, line, offset, from, to);
}
 ...
 */
export function findErrorNode(expr: SourcedSyntaxNode): SourcedSyntaxNode | undefined {
