...
export function Progress_createScriptFunctions(settings: ISettings): IScriptFunctions {
	...
	function increment(vals: IDynamicWeight, context: IScriptFnContext): IDynamicWeight;
	...
	function decrement(vals: IDynamicWeight, context: IScriptFnContext): IDynamicWeight;
...
function sum(...args: LogicResult[]): Quantity {
	...
	return flat.reduce<Quantity>(
		(acc, a) => Weight.operateAfterNormalized(undefined, acc, a, (x, y) => x + y),
		0,
	);
}
...
function min(...args: LogicResult[]): Quantity {
	...
	return flat.reduce<Quantity>((acc, a) => (Weight.lt(a, acc) ? a : acc), flat[0]);
}
...
function max(...args: LogicResult[]): Quantity {
	...
	return flat.reduce<Quantity>((acc, a) => (Weight.lt(acc, a) ? a : acc), flat[0]);
}
