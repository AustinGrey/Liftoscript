...
export function Reps_display(sets: ISet[], isNext: boolean = false): string {
  if (Reps_areSameReps(sets, isNext)) {
    ...
    const arr = sets.map((s) =>
      isNext ? Reps_displayReps(s) : Reps_displayCompletedReps(s),
    );
...
export function Reps_setToDisplaySet(
  set: ISet,
  isNext: boolean,
  settings: ISettings,
): IDisplaySet {
  ...
  return {
    ...
    isInRange:
      set.minReps != null
        ? set.completedReps != null && set.completedReps >= set.minReps
        : undefined,
  };
}
  ...
  if (lastSet == null) {
    ...
    if (isWarmup) {
      ...
      lastSet = {
        ...
        originalWeight:
          lastSet.originalWeight ?? lastSet.weight ?? lastSet.completedWeight,
...
export function Reps_isSameSet(set1: ISet, set2: ISet): boolean {
  return (
    Weight_eqNull(set1.weight, set2.weight) &&
    set1.completedReps === set2.completedReps &&
    set1.rpe === set2.rpe
  );
}
...
export function Reps_areSameReps(sets: ISet[], isNext: boolean): boolean {
  ...
  if (sets.length > 0) {
    return sets.every(
      (s) =>
        (isNext ? s.reps : s.completedReps) != null &&
        (isNext ? s.reps : s.completedReps) === firstRep,
    );
  } else {
    return false;
  }
...
export function Reps_isInRangeCompleted(sets: ISet[]): boolean {
  return (
    sets.some((s) => s.minReps != null) &&
    sets.every((s) => Reps_isInRangeCompletedSet(s))
  );
}
...
export function Reps_avgUnilateralCompletedReps(set: ISet): number | undefined {
  if (set.isUnilateral) {
    return Math.round(
      ((set.completedReps ?? 0) + (set.completedRepsLeft ?? 0)) / 2,
    );
  } else {
    return set.completedReps;
  }
...
export function Reps_setVolume(set: ISet, unit: IUnit): IWeight {
  const totalReps =
    set.isUnilateral || set.completedRepsLeft != null
      ...
      : (set.completedReps ?? 0);
  return Weight_multiply(
    set.completedWeight ?? set.weight ?? Weight_build(0, unit),
    totalReps,
  );
}
...
export function Reps_findNextEntryAndSetIndex(
  ...
  return {
    entryIndex: historyRecord.entries.indexOf(nextEntry),
    setIndex: nextSet,
  };
}

export function Reps_groupConsecutive<T>(
  items: T[],
  keyFn: (item: T) => string,
): [T, number][] {
...
export function Reps_volume(sets: ISet[], unit: IUnit): IWeight {
  return Weight_convertTo(
    sets.reduce(
      (memo, set) => Weight_add(memo, Reps_setVolume(set, unit)),
      Weight_build(0, unit),
    ),
    unit,
  );
