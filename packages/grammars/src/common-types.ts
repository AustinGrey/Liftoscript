...
export interface IScriptFunctions {
	...
	sum(
		...vals: (number | number[] | IWeight | IWeight[] | IDynamicWeight | IDynamicWeight[])[]
	): Quantity;
	min(
		...vals: (number | number[] | IWeight | IWeight[] | IDynamicWeight | IDynamicWeight[])[]
	): Quantity;
...
export const TExerciseDataValue = z.strictObject({
	...
	equipment: z.record(z.string(), z.union([z.string(), z.undefined()])).optional(),
	notes: z.string().optional(),
	muscleMultipliers: z.record(TMuscle, z.union([z.number(), z.undefined()])).optional(),
 ...
 */
export type NodeResult<T> = IEither<T, Oneⵜ<SourcedSyntaxError>>;
