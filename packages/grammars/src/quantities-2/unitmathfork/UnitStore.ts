...
import { normalize, isValidUnitName } from "./utils";
...
export function createUnitStore<T>(options: RequiredOptions<T>): UnitStore<T> {
	...
	while (true) {
		...
		for (const unitDefKey of Object.keys(originalDefinitions.units)) {
			...
			if (
				typeof unitDef !== "string" &&
				unitDef.prefixGroup &&
				!defs.prefixGroups.hasOwnProperty(unitDef.prefixGroup)
			) {
				...
				try {
					if (unitDef.hasOwnProperty("value")) {
						if (unitDef && typeof unitDef.value === "string") {
							...
							throw new TypeError(
								`Unit definition for '${unitDefKey}' must be an object with a value property where the value is a string or a two-element array.`,
							);
						}
					} else {
						throw new TypeError(
			...
			if (!skipThisUnit) {
				...
				unitAndAliases.forEach(newUnitName => {
					...
					if (!isValidUnitName(newUnitName) && newUnitName !== "") {
						throw new SyntaxError(
							`Unit name contains non-alphanumeric characters or begins with a number: '${newUnitName}'`,
						);
					}
					const newUnit: UnitPropsExtended<T> = {
						...
						prefixGroup: (unitDef.prefixGroup && defs.prefixGroups[unitDef.prefixGroup]) || {
							"": 1,
						},
		...
		if (unitsSkipped.length === 0) break;
			...
			throw new Error(
				`Could not create the following units: ${unitsSkipped.join(", ")}. Reasons follow: ${reasonsSkipped.join(" ")}`,
			);
		}
	}
	...
	if (options.system !== "auto") {
		if (!originalDefinitions.systems.hasOwnProperty(options.system)) {
			throw new Error(
				`Unknown unit system ${options.system}. Available systems are: auto, ${Object.keys(originalDefinitions.systems).join(", ")} `,
			);
		}
	}
		...
		if (unit.formatPrefixes) {
			for (let i = 0; i < unit.formatPrefixes.length; i++) {
				...
				if (!unit.prefixGroup.hasOwnProperty(s)) {
					throw new Error(
						`In unit ${unit.name}, common prefix ${s} was not found among the allowable prefixes`,
					);
				}
			}
		}
