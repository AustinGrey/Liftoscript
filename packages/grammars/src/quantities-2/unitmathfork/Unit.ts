...
import { symIsDefaultFun } from "./types";
...
let _config = function _config<T = number>(options: RequiredOptions<T>): UnitFactory<T> {
	...
	if (
		options.formatPrefixDefault &&
		!validFormatPrefixDefault.includes(options.formatPrefixDefault)
	) {
		throw new Error(
			`Invalid option for formatPrefixDefault: '${options.formatPrefixDefault}'. Valid options are ${validFormatPrefixDefault.join(", ")}`,
		);
	}
	...
	if (oneRequiredTypeFnsPresent) {
		if (!allRequiredTypeFnsPresent) {
			throw new Error(
				`You must supply all required custom type functions: ${requiredTypeFns.join(", ")}`,
			);
		}
		...
		if (options.autoPrefix) {
			...
			if (!allPrefixRequiredTypeFnsPresent) {
				throw new Error(
					`The following custom type functions are required when prefix is true: ${prefixRequiredTypeFns.join(", ")}`,
				);
			}
		}
	}
	...
	class _Unit implements Unit<T> {
		...
		constructor(value?: number | T | null | string | ParsedUnit<T>, unitString?: string) {
			...
			if (typeof value === "undefined" && typeof unitString === "undefined") {
				...
				throw new TypeError(
					"To construct a unit, you must supply a single string, two strings, a number and a string, or a custom type and a string.",
				);
			}
			...
			this.value =
				parseResult.value === undefined || parseResult.value === null
					? null
					: denormalize(
							this.unitList,
							normalize(parseResult.unitList, parseResult.value, options.type),
							options.type,
						);
			this.fixed = false;
		}
		...
		simplify(simplifyOptions: SimplifyOptions<T>) {
			...
			if (ok) {
				...
				if (this.value !== null) {
					simplifiedUnit.value = denormalize(
						simplifiedUnit.unitList,
						normalize(this.unitList, this.value, options.type),
						options.type,
					);
				} else {
		...
		compare(other: Unit<T> | string | T) {
			if (
				!options.type.conv[symIsDefaultFun] &&
				(options.type.gt[symIsDefaultFun] || options.type.lt[symIsDefaultFun])
			) {
				throw new Error(
					`When using custom types, compare requires a type.gt and a type.lt function`,
				);
			}
			other = _convertParamToUnit(other);
		...
		toString(formatOptions?: FormatOptions<T>) {
			...
			let str = "";
			if (
				typeof simp.value === "number" &&
				extendedOptions.formatter[symIsDefaultFun] &&
				extendedOptions.precision > 0
			) {
	...
	function _convertParamToUnit(
		otherOrValue: Unit<T> | ParsedUnit<T> | string | T,
		unit?: string,
	): Unit<T> {
	...
	function _add(unit1: Unit<T>, unit2: Unit<T>) {
		if (
			unit1.value === null ||
			unit1.value === undefined ||
			unit2.value === null ||
			unit2.value === undefined
		) {
			throw new Error(
				`Cannot add ${unit1.toString()} and ${unit2.toString()}: both units must have values`,
			);
		}
		if (!unit1.equalsQuantity(unit2)) {
			throw new Error(
				`Cannot add ${unit1.toString()} and ${unit2.toString()}: dimensions do not match`,
			);
		}
		const result = _clone(unit1);
		result.value = denormalize(
			unit1.unitList,
			options.type.add(
				normalize(unit1.unitList, unit1.value, options.type),
				normalize(unit2.unitList, unit2.value, options.type),
			),
			options.type,
		);
		return result;
	...
	function _sub(unit1: Unit<T>, unit2: Unit<T>): Unit<T> {
		if (
			...
			throw new Error(
				`Cannot subtract ${unit1.toString()} and ${unit2.toString()}: both units must have values`,
			);
		}
		if (!unit1.equalsQuantity(unit2)) {
			throw new Error(
				`Cannot subtract ${unit1.toString()} and ${unit2.toString()}: dimensions do not match`,
			);
		}
		const result = _clone(unit1);
		result.value = denormalize(
	...
	function _mul(unit1: Unit<T>, unit2: Unit<T>) {
		...
		if (unit1.value !== null || unit2.value !== null) {
			...
			const val1 =
				unit1.value === null
					? normalize(unit1.unitList, one, options.type)
					: normalize(unit1.unitList, unit1.value, options.type);
			const val2 =
				unit2.value === null
					? normalize(unit2.unitList, one, options.type)
					: normalize(unit2.unitList, unit2.value, options.type);
	...
	function _split(unit: Unit<T>, units: string[]): Unit<T>[] {
		if (
			!options.type.conv[symIsDefaultFun] &&
			(options.type.round[symIsDefaultFun] || options.type.trunc[symIsDefaultFun])
		) {
			throw new Error(
				`When using custom types, split requires a type.round and a type.trunc function`,
			);
		}
		...
		for (let i = 0; i < result.length; i++) {
			testSum = options.type.add(
				testSum,
				normalize(result[i].unitList, result[i].value!, options.type),
			);
		}
	...
	function _abs(unit: Unit<T>) {
		...
		if (result.value !== null) {
			result.value = denormalize(
				result.unitList,
				options.type.abs(normalize(result.unitList, result.value, options.type)),
				options.type,
			);
		}
		return result;
	...
	function _to(unit: Unit<T>, valuelessUnit: Unit<T>) {
		...
		result.value = denormalize(
			result.unitList,
			normalize(unit.unitList, value, options.type),
			options.type,
		);
		return result;
	}
	...
	function _choosePrefix(unit: Unit<T>, prefixOptions: Required<PrefixOptions<T>>) {
		...
		if (
			options.type.le(options.type.abs(unit.value), prefixOptions.prefixMax) &&
			options.type.ge(options.type.abs(unit.value), prefixOptions.prefixMin)
		) {
		...
		let prefixes =
			piece.unit.formatPrefixes ??
			(prefixOptions.formatPrefixDefault === "all"
				? Object.keys(piece.unit.prefixGroup)
				: undefined);
		...
		result.value = denormalize(
			result.unitList,
			normalize(unit.unitList, unit.value, options.type),
			options.type,
		);
	 ...
	 */
	function _withDefaults<OptionType extends SimplifyOptions<T> | FormatOptions<T>>(
