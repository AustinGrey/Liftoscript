...
import { type SourcedSyntaxNode, parseBound as originalParseBound } from "@/utils/lezer.ts";
import {
	type QueryOptions,
...
export function* queryChildren<TTypes extends NodeNames_Logic>(
	...
	yield* originalQueryChildren(node, options) as Generator<TypedLogicNode<TTypes>>;
}
...
export function* queryTree(
	...
	yield* originalQueryTree(node, where as Parameters<typeof originalQueryTree>[1]) as Generator<
		TypedLogicNode<NodeNames_Logic>
	>;
}
...
export function parseBound(script: string): TypedLogicNode<NodeNames_Logic> {
	return originalParseBound(LiftoscriptParser, script) as TypedLogicNode<NodeNames_Logic>;
}
