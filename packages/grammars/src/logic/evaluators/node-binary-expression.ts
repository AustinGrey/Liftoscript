...
import { type LogicResult, type LogicResultSingular, type Quantity } from "@/logic/types.ts";
import { type IWeight, percentORM, TDynamicWeight, TWeight } from "@/quantities/weight.ts";
...
function binaryOpMaybeArray(
	left: LogicResult,
	right: LogicResult,
	o: (leftSingular: LogicResultSingular, rightSingular: LogicResultSingular) => LogicResultSingular,
): LogicResult {
...
function binaryMathOp(
	...
	throw new Error(`Can't apply operation ${operator} to ${typeof a} and ${typeof b}`);
}
...
function binaryBooleanOp(
	...
	function coerceToBoolean(val: LogicResultSingular): boolean {
		...
		if (is(TDynamicWeight, val) && coercion.dynamicWeight) return coercion.dynamicWeight;
