import type { NodeNames_Logic, TypedLogicNode } from "@/logic/parsing/guards.ts";
import { type IDynamicWeight, type IWeight, TDynamicWeight, TWeight } from "@/quantities/weight.ts";
import { z } from "zod";
import type { ILiftoscriptEvaluatorUpdate, LogicResult, Quantity } from "@/logic/types.ts";
...
export type EvaluateTools = {
	 ...
	 */
	getState: (key: string, relatedNode: SourcedSyntaxNode, index?: number) => Quantity | undefined;
	/**
	...
	updateState: (
		key: string,
		value: Quantity | undefined | ((current: Quantity | undefined) => Quantity | undefined),
	...
	getGlobal: <TKey extends keyof IScriptBindings>(key: TKey) => IScriptBindings[TKey];
	updateGlobal: <TKey extends keyof IScriptBindings>(
		key: TKey,
		value:
			| IScriptBindings[TKey]
			// A function that takes in the current value.
			| ((currentValue: Readonly<IScriptBindings[TKey]>) => IScriptBindings[TKey]),
	) => void;
	/**
	 ...
	 * @param key The key of the state variable
	 */
	getVar: (key: string) => Quantity | undefined;
	/**
